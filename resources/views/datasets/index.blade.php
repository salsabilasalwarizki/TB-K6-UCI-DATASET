...
@section('meta_desc', 'Explore our collection of machine learning datasets')
            ...
            <div class="row align-items-center">
                <div class="col-lg-8">
                    ...
                    <p class="lead mb-0 opacity-90">
                        Discover {{ number_format($datasets->total() ?? 0) }} datasets for your machine learning projects
                    </p>
                </div>
                <div class="col-lg-4 text-lg-end mt-3 mt-lg-0">
                    <a href="{{ route('contribute.policy') }}" class="btn btn-light btn-lg fw-semibold">
                        ...
                        <form method="GET" action="{{ route('datasets.index') }}" id="filterForm">
                            ...
                            <div class="filter-group mb-3 pb-3 border-bottom">
                                ...
                                <div id="kwCollapse" class="collapse show">
                                    ...
                                    <div class="keywords-scroll" style="max-height: 120px; overflow-y: auto;">
                                        @forelse($keywords->take(15) as $keyword)
                                        <div class="form-check form-check-sm keyword-item">
                                            <input class="form-check-input" type="checkbox" 
                                                   name="keywords[]" value="{{ $keyword->keyword_id }}"
                                                   id="kw_{{ $keyword->keyword_id }}"
                                                   {{ in_array($keyword->keyword_id, request('keywords', [])) ? 'checked' : '' }}>
                                            <label class="form-check-label small" for="kw_{{ $keyword->keyword_id }}">
                                                {{ $keyword->keyword_name }}
                                                <span class="badge bg-light text-dark ms-1">{{ $keyword->datasets_count ?? 0 }}</span>
                                            </label>
                                        </div>
                                        @empty
                            ...
                            <div class="filter-group mb-3 pb-3 border-bottom">
                                ...
                                <div id="typeCollapse" class="collapse show">
                                    @php $dataTypes = ['Multivariate','Text','Image','Time-Series','Sequential','Tabular','Relational','Domain-Theory','Data-Generator','Univariate']; @endphp
                                    @foreach($dataTypes as $type)
                                    <div class="form-check form-check-sm">
                                        <input class="form-check-input" type="radio" name="data_type" 
                                               value="{{ $type }}" id="dt_{{ str_replace('-', '_', strtolower($type)) }}"
                                               {{ request('data_type') == $type ? 'checked' : '' }}>
                                        <label class="form-check-label small" for="dt_{{ str_replace('-', '_', strtolower($type)) }}">
                                            {{ $type }}
                                            <span class="badge bg-light text-dark ms-1">{{ $stats['data_type_counts'][$type] ?? 0 }}</span>
                                        </label>
                                    </div>
                                    @endforeach
                                </div>
                                ...
                                <div id="areaCollapse" class="collapse show">
                                    ...
                                    <div class="areas-scroll" style="max-height: 120px; overflow-y: auto;">
                                        ...
                                        <div class="form-check form-check-sm subject-item">
                                            <input class="form-check-input" type="checkbox" 
                                                   name="subject_area[]" value="{{ $area->area_name }}"
                                                   id="sa_{{ $area->area_id }}"
                                                   {{ in_array($area->area_name, request('subject_area', [])) ? 'checked' : '' }}>
                                            <label class="form-check-label small" for="sa_{{ $area->area_id }}">
                                                {{ $area->area_name }}
                                                <span class="badge bg-light text-dark ms-1">{{ $area->datasets_count ?? 0 }}</span>
                                            </label>
                                        </div>
                                        @empty
                            ...
                            <div class="filter-group mb-3 pb-3 border-bottom">
                                ...
                                <div id="taskCollapse" class="collapse show">
                                    @php $taskTypes = ['Classification','Regression','Clustering','Causal-Discovery','Relational-Learning','Other']; @endphp
                                    @foreach($taskTypes as $task)
                                    <div class="form-check form-check-sm">
                                        <input class="form-check-input" type="radio" name="task_type" 
                                               value="{{ $task }}" id="task_{{ str_replace('-', '_', strtolower($task)) }}"
                                               {{ request('task_type') == $task ? 'checked' : '' }}>
                                        <label class="form-check-label small" for="task_{{ str_replace('-', '_', strtolower($task)) }}">
                                            {{ $task }}
                                            <span class="badge bg-light text-dark ms-1">{{ $stats['task_type_counts'][$task] ?? 0 }}</span>
                                        </label>
                                    </div>
                                    @endforeach
                                </div>
                            ...
                            <div class="filter-group mb-3 pb-3 border-bottom">
                                ...
                                <div class="d-flex gap-2 mb-2">
                                    <input type="number" class="form-control form-control-sm" name="instances_min" 
                                           placeholder="Min" value="{{ request('instances_min') }}" min="0">
                                    <input type="number" class="form-control form-control-sm" name="instances_max" 
                                           placeholder="Max" value="{{ request('instances_max') }}" min="0">
                                </div>
                                ...
                                <div class="d-flex justify-content-between small text-muted">
                                    <span>0</span>
                                    <span id="instancesValue">{{ number_format(request('instances_range', $stats['max_instances'] ?? 100000)) }}</span>
                            ...
                            <div class="filter-group mb-3 pb-3 border-bottom">
                                ...
                                <div class="d-flex gap-2 mb-2">
                                    <input type="number" class="form-control form-control-sm" name="features_min" 
                                           placeholder="Min" value="{{ request('features_min') }}" min="0">
                                    <input type="number" class="form-control form-control-sm" name="features_max" 
                                           placeholder="Max" value="{{ request('features_max') }}" min="0">
                                </div>
                                ...
                                <div class="d-flex justify-content-between small text-muted">
                                    <span>0</span>
                                    <span id="featuresValue">{{ number_format(request('features_range', $stats['max_features'] ?? 1000)) }}</span>
                                ...
                                <div class="d-flex flex-wrap gap-2">
                                    <label class="form-check form-check-sm">
                                        <input class="form-check-input" type="checkbox" name="has_missing" value="1"
                                               id="hasMissing" {{ request('has_missing') ? 'checked' : '' }}>
...
@endif
            ...
            <main class="col-lg-9 col-md-8">
                ...
                <div class="toolbar d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4 pb-3 border-bottom">
                    ...
                    <div class="dropdown">
                        <button class="btn btn-outline-secondary btn-sm dropdown-toggle" type="button" 
                            ...
                            Sort: {{ $sortLabels[request('sort', 'view_count')] ?? '# Views' }}
                        </button>
                        <ul class="dropdown-menu dropdown-menu-sm">
                            <li><h6 class="dropdown-header">Sort By</h6></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item {{ request('sort') == 'view_count' ? 'active' : '' }}" 
                                   href="{{ request()->fullUrlWithQuery(['sort' => 'view_count', 'order' => 'desc']) }}">
                                <i class="bi bi-eye me-2"></i>Most Viewed
                            </a></li>
                            <li><a class="dropdown-item {{ request('sort') == 'download_count' ? 'active' : '' }}" 
                                   href="{{ request()->fullUrlWithQuery(['sort' => 'download_count', 'order' => 'desc']) }}">
                                <i class="bi bi-cloud-download me-2"></i>Most Downloaded
                            </a></li>
                            <li><a class="dropdown-item {{ request('sort') == 'citation_count' ? 'active' : '' }}" 
                                   href="{{ request()->fullUrlWithQuery(['sort' => 'citation_count', 'order' => 'desc']) }}">
                            ...
                            <li><a class="dropdown-item {{ request('sort') == 'created_at' ? 'active' : '' }}" 
                                   href="{{ request()->fullUrlWithQuery(['sort' => 'created_at', 'order' => 'desc']) }}">
                ...
                <!-- Datasets Grid/List -->
                <div class="datasets-container {{ request('view') == 'grid' ? 'row g-3' : '' }}" id="datasetsContainer">
                    @forelse($datasets as $dataset)
                    <article class="dataset-card card h-100 border-0 shadow-sm hover-lift {{ request('view') == 'grid' ? 'col-md-6 col-lg-4' : '' }}" 
                        ...
                        <div class="card-body p-3">
                            <div class="d-flex gap-3">
                                ...
                                <div class="flex-shrink-0">
                                    <div class="dataset-thumbnail position-relative rounded overflow-hidden" 
                                        ...
                                        <span class="position-absolute top-0 end-0 badge {{ $dataset->status === 'pending' ? 'bg-warning' : 'bg-secondary' }} rounded-pill" 
                                ...
                                <div class="flex-grow-1 min-w-0">
                                    ...
                                    <div class="badges mb-2">
                                        ...
                                        <span class="badge bg-info-subtle text-info border border-info-subtle me-1">{{ $dataset->data_type }}</span>
                                        @endif
                                        @if($dataset->task_type)
                                        <span class="badge bg-success-subtle text-success border border-success-subtle">{{ $dataset->task_type }}</span>
                                        @endif
                                    </div>
                                    
                                    <!-- Description -->
                                    <p class="card-text small text-muted mb-2 line-clamp-2">
                                        {{ Str::limit($dataset->abstract ?? $dataset->description, request('view') == 'grid' ? 60 : 100) }}
                                    </p>
                                    
                                    <!-- Stats -->
                                    <div class="stats d-flex flex-wrap gap-3 small text-muted mb-2">
                                        @if($dataset->num_instances !== null)
                                        <span title="Instances"><i class="bi bi-table me-1"></i>{{ $dataset->num_instances >= 1000000 ? number_format($dataset->num_instances / 1000000, 1) . 'M' : number_format($dataset->num_instances) }}</span>
                                        @endif
                                        @if($dataset->num_features !== null)
                                        <span title="Features"><i class="bi bi-grid-3x3-gap me-1"></i>{{ number_format($dataset->num_features) }}</span>
                                        @endif
                                        @if($dataset->num_classes !== null)
                                        <span title="Classes"><i class="bi bi-diagram-3 me-1"></i>{{ $dataset->num_classes }}</span>
                                        @endif
                                    ...
                                    <div class="meta small text-muted mb-2">
                                        ...
                                        <span><i class="bi bi-folder me-1"></i>{{ $dataset->subject_area }}</span>
                                        @endif
                                        @if($dataset->donated_date)
                                        <span class="ms-2"><i class="bi bi-calendar me-1"></i>{{ \Carbon\Carbon::parse($dataset->donated_date)->format('M Y') }}</span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="actions d-flex gap-2 mt-auto pt-2">
                                        ...
                                        <a href="{{ asset('storage/' . $defaultFile->file_path) }}" 
                                           ...
                                           title="Download {{ $defaultFile->original_filename ?? $defaultFile->filename }}">
                                            <i class="bi bi-download"></i>
                                        </a>
                                        @endif
                                        <button class="btn btn-sm btn-outline-secondary" 
                                                onclick="toggleCard({{ $dataset->dataset_id }})"
                            ...
                            <!-- Expanded Details -->
                            <div class="dataset-details collapse mt-3 pt-3 border-top" id="details-{{ $dataset->dataset_id }}">
                                <div class="row g-3">
                                    <!-- Variables Preview -->
                                    <div class="col-md-6">
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-sm table-borderless mb-0" style="font-size: 0.8rem;">
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <td class="py-1 pe-2">{{ $var->display_name ?? $var->variable_name }}</td>
                                                        <td class="py-1"><span class="badge bg-light border">{{ $var->variable_type }}</span></td>
                                                    </tr>
                                                    @endforeach
                                                </tbody>
                                            </table>
                                        ...
                                        <small class="text-muted">+{{ $dataset->variables->count() - 4 }} more</small>
                                        @endif
                                        @endif
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <p class="small mb-1"><strong>Purpose:</strong> {{ Str::limit($dataset->descriptionDetails->purpose, 80) }}</p>
                                        @endif
                                        @if($dataset->descriptionDetails->instances_represent)
                                        <p class="small mb-0"><strong>Instances:</strong> {{ Str::limit($dataset->descriptionDetails->instances_represent, 80) }}</p>
                                        @endif
                                        @endif
                                ...
                                <div class="d-flex gap-2 mt-3 pt-2 border-top">
                                    <button class="btn btn-sm btn-outline-primary" onclick="importInPython({{ $dataset->dataset_id }})">
                                        <i class="bi bi-code-slash me-1"></i>Python
                                    </button>
                                    <button class="btn btn-sm btn-outline-warning" onclick="showCitation({{ $dataset->dataset_id }}, '{{ addslashes($dataset->name) }}')">
                                        <i class="bi bi-quote me-1"></i>Cite
                                    </button>
                                    @if(auth()->check())
                                    <button class="btn btn-sm btn-outline-info" onclick="addToCollection({{ $dataset->dataset_id }})">
...
@endif
                        ...
                        @if ($datasets->hasMorePages())
                            <li class="page-item"><a class="page-link" href="{{ $datasets->nextPageUrl() }}" rel="next">›</a></li>
                        @else
                            <li class="page-item disabled"><span class="page-link">›</span></li>
                        @endif
                    </ul>
                ...
                <div class="text-center text-muted small mt-3">
                    Showing {{ $datasets->firstItem() ?? 0 }}–{{ $datasets->lastItem() ?? 0 }} of {{ $datasets->total() }} datasets
                </div>
